...
list_files_ftps()
{
	...
	#echo "${CURL} --config ${CFG} --ftp-ssl -k --silent --show-error --list-only ftp://$ftp_ip:${PORT}$ftp_path";

	${CURL} --config ${CFG} --ftp-ssl -k --silent --show-error ftp://$ftp_ip:${PORT}$ftp_path/ > ${DUMP} 2>&1
	RET=$?
